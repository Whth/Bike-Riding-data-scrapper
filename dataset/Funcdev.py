...
def cheek_local_phone_format() -> int:
    ...
    if os.path.exists("/dataset/local_phone.txt"):
        ...
        with open("/dataset/local_phone.txt", "r") as f:
            lineCounter = 0
            line = f.readline()
            while line != '':
                line = line.split()
...
def sync_phone_txt(add="/dataset/local_phone.txt"):
    """
...
def get_all_lines_count(filenameAdd='/dataset/local_phone.txt') -> int:
    """
...
def get_lines_with_content_Count(filenameAdd='/dataset/local_phone.txt') -> int:
    """
