...
class ttbikeSpider(CP):
    ...
    def _getCaptcha(self, phone):
        ...
        webEcho = self._session.post(get_captcha_url, headers=self._headers, data=json.dumps(get_captcha_data),
                                     timeout=self.timeout)
        if "imageCaptcha" in webEcho.text:  # 获取短信验证码前需要验证图片验证码，如果response包含验证码字段，则正确，否则异常
            img = json.loads(webEcho.text)["data"]["imageCaptcha"]
            ...
            print('图片验证码获取失败, 原因:', webEcho.text)
    ...
    def _sendSMSCode2(self, phone):
        if self._captcha_status:  # 如果已经获取过验证码
            ...
            while fail_count < 5:
                ...
                r = self._session.post(get_SMSCode_url, headers=self._headers, data=json.dumps(get_SMSCode_data),
                                       timeout=self.timeout)
                if 'true' in r.text:
    ...
    def _getBikes(self, lng, lat, token):
        ...
        r = requests.post(get_bike_url, headers=self._headers, data=json.dumps(get_bike_data),
                          timeout=20)  # 此处超时最好设置大一点
        if 'bikeNo' in r.text:
